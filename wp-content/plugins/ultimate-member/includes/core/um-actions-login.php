...
function um_submit_form_errors_hook_login( $args ) {
	...
	if ( isset( $args['username'] ) && $args['username'] == '' ) {
		UM()->form()->add_error( 'username', __( 'Zadajte svoje používateľské meno alebo e-mail', 'ultimate-member' ) );
	}
	...
	if ( $args['user_password'] == '' ) {
		UM()->form()->add_error( 'user_password', __( 'Prosím vložte heslo', 'ultimate-member' ) );
	}
...
function um_after_login_submit( $args ) {
	...
	<div class="um-col-alt-b">
		<a href="<?php echo esc_url( um_get_core_page( 'password-reset' ) ); ?>" class="um-link-alt">
			<?php _e( 'Zabudli ste heslo?', 'ultimate-member' ); ?>
		</a>
	</div>
